(() => {
  ...
  async function read(path, push = false) {
    ...
    const pure = path.split('?')[0]; document.querySelectorAll('.archive-list .news-row').forEach(row => { const selected = row.getAttribute('href')?.split('?')[0] === pure; row.classList.toggle('selected', selected); if (selected) row.setAttribute('aria-current', 'page'); else row.removeAttribute('aria-current'); });
    ...
    } catch (_) { if (!signal.aborted) { pane.innerHTML = '<div class="empty">无法读取报道，请独立打开重试。</div>'; document.getElementById('reader-status').textContent = '报道读取失败'; } }
  }
  if (pane) {
    ...
    document.querySelector('.archive-list').addEventListener('click', event => {
      const link = event.target.closest('a.news-row'); if (!link || event.ctrlKey || event.metaKey || event.shiftKey || event.altKey || event.button !== 0 || !articlePath(link.getAttribute('href'))) return;
    ...
    function restore() { const selected = new URL(location.href).searchParams.get('article'); if (selected) { if (!media.matches) { const target = new URL(selected, location.origin); target.searchParams.set('return', cleanArchive()); location.replace(target); } else read(selected); } else { controller?.abort(); pane.innerHTML = '<div class="empty">选择一篇报道阅读</div>'; document.querySelectorAll('.archive-list .news-row').forEach(row => { row.classList.remove('selected'); row.removeAttribute('aria-current'); }); document.getElementById('reader-open').hidden = true; } }
