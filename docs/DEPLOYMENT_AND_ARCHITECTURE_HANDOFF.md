...
首页是**工作台**：**开屏第一眼就是当天要读的简报**，运行读数一律退到阅读面之后，自上而下：
...
6. **运行概览**：**默认收起**的 `<details id="overview">`，点标题展开。里面是数据面口径说明、KPI 行
   （台账条目 / 本期新增 / 实质更新 / 待读；待读 = 简报后更新中未标记已读的条数）、采集节律条
   （采集节律每 30 分钟 · :00 / :30、下一轮采集倒计时按访客本机时间算、简报窗口与下一期估算、
   新鲜度窗口）、近 7 日入库量折线（按条目**发布时间**分箱，不是采集时间；图下写明窗口外 / 无时间 /
   未来时间的条数）与本轮新增与更新动态列表。折叠是刻意的：这些是运行读数，不是每天要读的内容。
