...
- **额度耗尽后简报未翻译**：检查当日 `ai_usageday.used`、`lane_used` 和待执行 `enrich` 任务。常规任务按时段释放额度；事件、近期新闻和历史回填的日上限分别为总额的 40%、40% 和 10%，简报与交互请求保留剩余 10%。提高每日上限前先核对实际用量与翻译覆盖率，设置变更不会自动清除已用 token。
